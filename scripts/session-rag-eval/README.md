...
Use the real Echo Infer Desktop Electron conversation flow when you need to verify local config, license loading, file upload,
...
The desktop-flow script copies the user's `config.json` into an isolated temporary userDataDir and points the session
RAG sqlite database at a separate temporary path, so the app reads the normal provider settings without
...
```bash
pnpm eval:session-rag:desktop -- --case implicit-citrine-current-policy
pnpm eval:session-rag:desktop -- --case implicit-multi-doc-release-followup
pnpm eval:session-rag:desktop -- --case real-wiki-apollo-implicit-landing-site
pnpm eval:session-rag:desktop -- --case multi-turn-real-wiki-apollo-followup
pnpm eval:session-rag:desktop -- --case real-wiki-unrelated-capital
```
