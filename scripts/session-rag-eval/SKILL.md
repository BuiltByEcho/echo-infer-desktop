...
description: Run and debug Echo Infer Desktop session attachment RAG evaluation with synthetic and real long-file fixtures.
---
...
Prefer the Echo Infer Desktop conversation-flow harness for product validation because it exercises the real renderer, config,
...
```bash
pnpm eval:session-rag:desktop -- --case long-citrine-threshold
...
pnpm eval:session-rag:desktop -- --case unrelated-simple-math
