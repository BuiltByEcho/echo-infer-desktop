...
// 这行代码是解决 Windows 通知的标题和图标不正确的问题，标题会错误显示成 electron.app.Echo Infer Desktop
...
// 开发环境使用 echo-infer-dev:// 协议，避免和正式版冲突
const PROTOCOL_SCHEME = process.defaultApp ? 'echo-infer-dev' : 'echo-infer'
...
function createTray() {
  ...
  tray.setToolTip('Echo Infer Desktop')
...
if (!gotTheLock) {
  ...
  app.on('second-instance', async (event, commandLine, workingDirectory) => {
    ...
    const url = commandLine.find((arg) => arg.startsWith('echo-infer://') || arg.startsWith('echo-infer-dev://'))
  ...
  app.on('window-all-closed', () => {
    // Respect the macOS convention of keeping the app in memory after all windows close.
    // On Windows/Linux, closing the only window should quit instead of leaving a hidden tray process.
    if (process.platform !== 'darwin') {
      app.quit()
    }
  })
  ...
  app
    ...
    .then(async () => {
      ...
      if (process.platform !== 'darwin') {
        const url = process.argv.find((arg) => arg.startsWith('echo-infer://') || arg.startsWith('echo-infer-dev://'))
        if (url && mainWindow) {
          // 确保窗口加载完成后再处理 Deep Link
          if (mainWindow.webContents.isLoading()) {
            mainWindow.webContents.once('did-finish-load', () => {
