...
const cardConfigs: CardConfig[] = [
  { type: 'novice', icon: echoInferIcon, highlighted: true },
...
export function UserTypeCards({ onSelect, disabled }: UserTypeCardsProps) {
  ...
  const cardTexts = {
    novice: {
      ...
      description: t('Connect Echo Infer'),
    },
    expert: {
      title: t('Skip guide'),
              ...
              <Stack gap="sm" align="center" className="p-5 pt-8">
                ...
                >
                  <img
                    src={icon}
                    alt=""
                    className={highlighted ? 'w-9 h-9 rounded-md' : 'w-6 h-6'}
                    data-testid={type === 'novice' ? 'echo-infer-guide-card-logo' : undefined}
                  />
                </Flex>
