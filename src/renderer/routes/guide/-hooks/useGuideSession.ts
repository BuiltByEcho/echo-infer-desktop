  ...
  useEffect(() => {
    ...
    if (enterCompleted) {
      ...
      const configCompleteMsg = String(
        t(
          "You've already completed setup and can use Echo Infer normally.\n\nIf you have questions about your Echo Infer API key or model access, feel free to ask me here."
        )
      )
      setMessages([
        {
      ...
      const greeting = t(`## 👋 Hey! I'm Echo, your Echo Infer setup guide.

Echo Infer Desktop is a focused private AI chat client for your existing Echo Infer API key at **https://infer.builtbyecho.xyz/v1**.

### ✨ What this beta focuses on
- 🔐 **Private by default** — your chats and settings stay local on this device
- ⚡ **Echo Infer ready** — prefilled for the echo-private model
- 🧭 **Simple setup** — paste your API key once, then start chatting

---

**Let's get you connected.** Choose the setup path that fits you:`)
  ...
  const selectUserType = useCallback(
    async (type: UserType) => {
      ...
      const responseContent =
        type === 'novice'
          ? t(
              'Great — Echo Infer is already configured for the hosted endpoint. Paste your existing Echo Infer API key in Settings, then use the prefilled echo-private model to start chatting.'
            )
          : t(
              'Excellent. Echo Infer is already focused on the hosted provider. Open **Settings** to review the Echo Infer API key field and `echo-private` model before chatting.'
            )
  ...
  const renderCelebration = useCallback(async () => {
    ...
    await streamFixedMessage(
      t(
        "Awesome, you're all set! You can now start using Echo Infer.\n\nClick **New Chat** below to start chatting, or open **Settings** to review your Echo Infer API key and model. Enjoy!"
      ),
      [
  ...
  const markGuideCompleted = useCallback(async () => {
    ...
    if (hasLicense) {
      ...
      await streamFixedMessage(
        t("You're connected. Echo Infer features are ready once your API key is saved in Settings."),
        [
          {
  ...
  const onClaimStart = useCallback(async () => {
    ...
    await streamFixedMessage(t("We're waiting for you to finish the Echo Infer setup..."), [
      {
  ...
  const debugTriggerRoundLimit = useCallback(() => {
    ...
    fakeMessages.push({
      id: generateMessageId(),
      role: 'assistant',
      content: t('Welcome to Echo Infer!'),
      parts: [{ type: 'text', text: t('Welcome to Echo Infer!') }],
    })
