...
import { type CopilotDetail, type ImageSource, ModelProviderEnum, type Session } from '@shared/types'
...
import {
  ECHO_INFER_DEFAULT_BASE_URL,
  ECHO_INFER_DEFAULT_MODEL,
  ECHO_INFER_SETTINGS_PATH,
} from '@/utils/echoInferOnboarding'
...
function Index() {
  ...
  const echoInferNeedsCredential = useSettingsStore((s) => {
    const apiKey = s.providers?.[ModelProviderEnum.EchoInfer]?.apiKey
    return typeof apiKey === 'string' ? apiKey.trim().length === 0 : true
  })
  const echoInferBaseUrl = useSettingsStore(
    (s) => s.providers?.[ModelProviderEnum.EchoInfer]?.apiHost || ECHO_INFER_DEFAULT_BASE_URL
  )
  const echoInferDefaultModel = useSettingsStore(
    (s) => s.providers?.[ModelProviderEnum.EchoInfer]?.models?.[0]?.modelId || ECHO_INFER_DEFAULT_MODEL
  )
  ...
  return (
    <Page title="">
      <div className="p-0 flex flex-col h-full">
        ...
        {echoInferNeedsCredential && (
          <Box px="sm">
            <Stack
              gap="xs"
              p="md"
              className={clsx(
                'mb-md border border-solid border-chatbox-border-primary rounded-lg bg-white/40 dark:bg-zinc-900/40',
                widthFull ? 'w-full' : 'w-full max-w-4xl mx-auto'
              )}
            >
              <Text fw="600">{t('Paste your existing Echo Infer API key')}</Text>
              <Text size="xs" c="chatbox-tertiary">
                {t(
                  'Echo Infer Desktop is prefilled for {{baseUrl}} and {{model}}. Add your scoped desktop key, then click Check to verify the connection.',
                  {
                    baseUrl: echoInferBaseUrl,
                    model: echoInferDefaultModel,
                  }
                )}
              </Text>
              <Flex gap="xs" wrap="wrap">
                <Button size="xs" onClick={() => navigateToSettings(ECHO_INFER_SETTINGS_PATH)}>
                  {t('Open Echo Infer settings')}
                </Button>
                <Button size="xs" variant="subtle" onClick={() => navigateToSettings('/provider/echo-infer')}>
                  {t('Connection test uses echo-private')}
                </Button>
              </Flex>
            </Stack>
          </Box>
