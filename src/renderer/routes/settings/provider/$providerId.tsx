...
function ProviderSettings({ providerId }: { providerId: string }) {
  ...
  const isEchoInferProvider = baseInfo?.id === ModelProviderEnum.EchoInfer
    ...
    <Stack key={baseInfo.id} gap="xxl">
      ...
      <Stack gap="xl">
        ...
        {isEchoInferProvider && (
          <Stack gap="xxs">
            <Text span fw="600">
              {t('Paste your existing Echo Infer API key')}
            </Text>
            <Text span size="xs" c="chatbox-tertiary">
              {t(
                'Use the desktop-client scoped key from Echo Infer settings. The default host is https://infer.builtbyecho.xyz/v1 and Check tests echo-private before you start chatting.'
              )}
            </Text>
          </Stack>
        )}
