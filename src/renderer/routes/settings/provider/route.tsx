...
export function RouteComponent() {
  ...
  const providers = useMemo<ProviderInfo[]>(() => {
    ...
    // Echo Infer Desktop keeps the legacy Chatbox AI provider route available but hides it from the default provider list.
    const visibleSystemProviders = systemProviders.filter((p) => p.id !== ModelProviderEnum.ChatboxAI)
    const echoInfer = visibleSystemProviders.find((p) => p.id === ModelProviderEnum.EchoInfer)
    const others = visibleSystemProviders.filter((p) => p.id !== ModelProviderEnum.EchoInfer)
    return [...(echoInfer ? [echoInfer] : []), ...others, ...(customProviders || [])].map((p) => ({
      ...p,
      ...(providersMap?.[p.id] || {}),
    }))
